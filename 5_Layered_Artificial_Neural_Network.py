...
# ---------------------------------------------------------
# Function Name : _Calculate_Weighted_Sum
...
# ---------------------------------------------------------
def _Calculate_Weighted_Sum(inputs, weights, bias):
    weighted_sum = sum(weight * input_value for weight, input_value in zip(weights, inputs)) + bias
    return weighted_sum


# ---------------------------------------------------------
# Function Name : _Display_Multiplication_Details
...
#   weights -> List of weights
# ---------------------------------------------------------
def _Display_Multiplication_Details(inputs, weights):
...
# ---------------------------------------------------------
# Function Name : _Process_Hidden_Layer
...
# ---------------------------------------------------------
def _Process_Hidden_Layer(inputs, hidden_weights, hidden_biases):
    hidden_outputs = []
    ...
    for neuron_index in range(len(hidden_weights)):
        ...
        _Display_Multiplication_Details(inputs, current_weights)

        # Calculate weighted sum
        z_value = _Calculate_Weighted_Sum(inputs, current_weights, current_bias)
        print(f"  Step 2: Add all multiplication results and bias {current_bias}")
        print(f"    z = {z_value:.3f}")

        # Apply ReLU activation
        activated_output = _ReLU(z_value)
...
# ---------------------------------------------------------
# Function Name : _Process_Output_Layer
...
# ---------------------------------------------------------
def _Process_Output_Layer(hidden_outputs, output_weights, output_bias):
    print("\n================ OUTPUT LAYER ================\n")
    ...
    z_output = _Calculate_Weighted_Sum(hidden_outputs, output_weights, output_bias)
    print(f"  Step 2: Add all multiplication results and bias {output_bias}")
    print(f"    z = {z_output:.3f}")

    # Apply Sigmoid activation
    final_output = _Sigmoid(z_output)
...
# ---------------------------------------------------------
# Function Name : _ANN_Forward_Pass
...
def _ANN_Forward_Pass(inputs):
    ...
    hidden_outputs = _Process_Hidden_Layer(
        inputs,
        hidden_weights,
        hidden_biases
    )
    ...
    z_output, final_output = _Process_Output_Layer(
        hidden_outputs,
        output_weights,
        output_bias
    )
    ...
    # Display summary
    _Display_Network_Summary(hidden_outputs, final_output)
